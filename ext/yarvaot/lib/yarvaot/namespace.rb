...
# This module implies an existance of instance variable @namesapce.
module YARVAOT::Converter
	...
	def robject2csource obj, volatilep = false, name = nil
		...
		when Float
			name ||= @namespace.new obj.to_s
			init = sprintf 'rb_float_new(%g)', obj
		when Range
			from = robject2csource obj.begin, :volatile
			to   = robject2csource obj.end,   :volatile
		...
		when String
			if obj.empty?
				...
				# encoding object.
				if obj.ascii_only?
					name ||= @namespace.new obj
					aenc = Encoding.find 'US-ASCII'
					encn = robject2csource aenc, :volatile
				else
					name ||= @namespace.new obj.encoding.name + '_' + obj
					encn = robject2csource obj.encoding, :volatile
				end
				deps << encn
		...
		when Symbol
			...
			if str.bytesize <= @strmax
				...
				# rather than a VALUE.
				a = rstring2cstr str
				e = robject2csource str.encoding, :volatile
				...
				rstr = robject2csource str, :volatile
				init = 'rb_str_intern(%s)' % rstr
				deps << rstr
			end
		when Regexp
			opts = obj.options
			srcs = robject2csource obj.source, :volatile
			name ||= @namespace.new srcs
