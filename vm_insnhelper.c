...
static VALUE
vm_check_keyword(lindex_t kw_bits_index,
		 rb_num_t keyword_index,
		 const VALUE *reg_ep)
{
    const VALUE kw_bits = *(reg_ep - kw_bits_index);
	...
	return rb_hash_has_key(kw_bits, INT2FIX(keyword_index)) ?
	    Qfalse : Qtrue;
    }
}

static VALUE
vm_find_or_create_class(ID id,
			ISEQ class_iseq,
			rb_num_t flags,
			VALUE cbase,
			VALUE super)
